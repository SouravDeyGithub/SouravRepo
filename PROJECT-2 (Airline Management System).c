...
struct sourav_airline
{
	...
	int seat_num;
	char email[15];
	struct sourav_airline *following;
}
*begin, *stream;
struct sourav_airline *dummy;
...
void main()
{
	...
	do
	{
		...
		printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   welcome to Sourav's airline system                   ");
		printf("\n\t\t   *******************************************************************");
...
void reserve(int x)
{
	...
	if (begin == NULL)
	{
		begin = stream = (struct sourav_airline*)malloc(sizeof(struct sourav_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct sourav_airline *)malloc(sizeof(struct sourav_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
		printf("\n\t Ticket booking succesful!");
		printf("\n\t your seat number is: Seat A-%d", x);
		stream->seat_num = x;
		return;
	}
...
void savefile()
{
	FILE *fpointer = fopen("sourav records", "w");
	if (!fpointer)
	{
	...
	printf("\n\n\t Details have been saved to a file (sourav records)");
	fclose(fpointer);
}
